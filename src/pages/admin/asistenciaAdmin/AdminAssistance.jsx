...
export default function AdminAssistance() {
  ...
  return (
    <div className="anim-fade-in" style={{ maxWidth: 1000 }}>
      {/* Titulo y cantidad de solicitudes  */}
          ...
          <div key={a.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              <div className='div-asistencia-info' style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                {a.usuarioNombre != "Visitante sin registro" && <span style={{ fontWeight: 700, fontSize: '1rem' }}>{a.usuarioNombre}</span>}
                {a.usuarioNombre == "Visitante sin registro" && <span style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--accent)' }}>{a.usuarioNombre}</span>}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
              {a.estado === 'pendiente' && (
                <button className="btn btn-success btn-sm" onClick={() => updateAssistance(a.id, { estado: 'atendido', atendidoPor: 'Prof. en turno' })} id={`btn-assist-done-${a.id}`}>
                  Marcar atendido
                </button>
              )}
            </div>
